...
export function DashboardSidebar({ active, eyebrow, items }: DashboardSidebarProps) {
  return (
    <>
      <div className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 px-3 py-3 backdrop-blur xl:hidden">
        <Link className="mb-3 flex items-center gap-3" href="/">
          <span className="grid size-10 place-items-center rounded-xl bg-[#0B8F47] text-lg font-black text-white">A</span>
          <span>
            <span className="block text-base font-bold text-slate-950">AnimKart OS</span>
        ...
        <nav className="-mx-3 flex gap-2 overflow-x-auto px-3 pb-1">
          {items.map((item) => {
            ...
            const className = isActive
              ...
              : "inline-flex h-10 shrink-0 items-center gap-2 rounded-full border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-700";
